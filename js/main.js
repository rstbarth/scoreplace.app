    ...
    },
    {
      ...
      content: '<div style="margin-bottom:1rem;">' +
        ...
        '<ul style="margin:4px 0 8px 18px;font-size:0.82rem;">' +
          '<li><b>Cloud Function</b>: detecta <code>session.casualMatchId</code> (novo campo, alternativo ao <code>tournamentId</code>) e, em vez de mutar o placar direto, enfileira <code>{action, nonce, uid, ts}</code> em <code>casualMatches/{id}.pendingRemoteActions[]</code> via transação (cap de 20 pra limitar memória). Re-valida que <code>session.uid === match.createdBy</code> — só o criador da partida pode controlar.</li>' +
          '<li><b>Client onSnapshot</b>: listener já existente em <code>bracket-ui.js</code> agora também detecta <code>pendingRemoteActions</code>. Apenas o cliente que é o <code>createdBy</code> processa (evita race entre múltiplos participantes assistindo). Cada action vira chamada a <code>_addPoint(1|2)</code> ou <code>_liveScoreFinish()</code> — reusa 100% do motor existente. Após aplicar, limpa a fila. Dedup via <code>_appliedRemoteNonces</code> Set (snapshot pode disparar 2x em reconexão).</li>' +
          '<li><b>Trade-off consciente</b>: requer o celular do organizador com a tela de placar ao vivo aberta. Se fechar, ações acumulam e aplicam catch-up quando reabrir (até 20 em fila). Aviso amarelo no modal deixa isso explícito.</li>' +
        '</ul>' +
        '<p><b>Ações casuais</b>: <code>point_team1</code>, <code>point_team2</code>, <code>finish</code>, <code>status</code>. Mais enxuto que a versão torneio porque o motor do app cuida de tudo — watch só sinaliza "ponto pro lado X". Response JSON carrega o <code>spoken</code> apropriado ("6-4. Game atual 30-15.") pro Atalho anunciar por voz.</p>' +
        '<p><b>UX</b>: botão "⌚" no header da tela de placar ao vivo (só aparece pro criador da partida casual). Modal é o mesmo do v0.15.39 mas com textos ajustados: banner amarelo de alerta, sem dropdown de partida (sessão casual é amarrada a uma única partida), lista de ações filtrada. Rules do <code>remoteSessions</code> relaxadas pra aceitar <code>casualMatchId</code> como alternativa a <code>tournamentId</code>.</p>' +
        '<p><b>Deploy manual necessário</b>: <code>firebase deploy --only functions:remoteScore</code> + <code>firebase deploy --only firestore:rules</code>. Arquivos: <code>js/remote-control.js</code> (extensão polimórfica), <code>js/views/bracket-ui.js</code> (listener + botão), <code>functions/index.js</code> (branch casual + <code>_summarizeCasualState</code>), <code>firestore.rules</code>, <code>js/store.js</code>, <code>sw.js</code>, <code>index.html</code>.</p>' +
        '</div>' +
        '<div style="margin-bottom:1rem;">' +
        '<div style="font-weight:700; color:var(--text-bright); font-size:0.9rem; margin-bottom:6px;">v0.15.39-alpha <span style="color:var(--text-muted); font-weight:400; font-size:0.75rem;">(Abril 2026)</span></div>' +
        '<p><b>⌚ Controle de placar pelo Apple Watch (via Apple Shortcuts).</b> Primeiro MVP de controle remoto do placar sem sacar o celular. Caso de uso: organizador caminha pela arena, tem uma partida marcada como "ativa", e dá um toque no Watch pra incrementar ponto / finalizar. Arquitetura em 3 camadas:</p>' +
        '<ul style="margin:4px 0 8px 18px;font-size:0.82rem;">' +
          '<li><b>Client</b> (<code>js/remote-control.js</code>): modal em <code>#bracket</code> gera token de 48 chars hex (192 bits entropia) + cria doc em <code>remoteSessions/{token}</code>. Token é o próprio ID do doc — conhecer o ID == ter o segredo. Validade 24h. LocalStorage persiste entre reloads.</li>' +
          '<li><b>Firestore rules</b>: <code>remoteSessions</code> aceita create/read/update/delete só do owner (uid+email amarrados ao auth context); listagem bloqueada — força acesso direto por token.</li>' +
          '<li><b>Cloud Function</b> (<code>remoteScore</code>, onRequest HTTPS): Apple Shortcut faz POST com <code>{token, action, ...}</code>; função lê sessão via Admin SDK, re-valida <code>email ∈ tournament.adminEmails</code> (revoga acesso imediato se organizador perdeu privilégios), aplica ação in-place no match e salva via <code>tRef.set(..., {merge:true})</code>.</li>' +
        '</ul>' +
        '<p>Ações suportadas no MVP: <code>inc_p1</code>/<code>inc_p2</code> (+1 ponto), <code>dec_p1</code>/<code>dec_p2</code> (desfazer), <code>set_score</code> (definir placar exato com <code>p1</code>+<code>p2</code>), <code>finalize</code> (confirma vencedor pelo placar atual), <code>status</code> (read-only). Response JSON traz <code>summary</code> e <code>spoken</code> — este último pensado pra "Falar texto" no Atalho ("6 a 4") pro Watch anunciar por voz.</p>' +
        '<p><b>UX</b>: botão "⌚ Apple Watch" na grade de ações do <code>#bracket</code> (visível só pra organizadores, só depois do sorteio). Modal mostra token, URL, QR code do token (pra apontar câmera do iPhone e copiar), dropdown pra trocar partida ativa em tempo real, e instruções passo-a-passo de como criar o Atalho com "Obter conteúdo do URL" → POST → JSON body. Botão "🛑 Encerrar sessão" deleta o doc no Firestore imediatamente.</p>' +
        '<p><b>Limitações MVP</b>: (1) só placar numérico simples — GSM (sets/games/tiebreak) fica como v2; (2) não auto-finaliza empate via remote (organizador ajusta no web); (3) função precisa de deploy manual via <code>firebase deploy --only functions:remoteScore</code> + rules via <code>firebase deploy --only firestore:rules</code> — até deployarem, o botão abre o modal mas o Shortcut retornaria 404. Arquivos: <code>js/remote-control.js</code> (novo), <code>functions/index.js</code>, <code>firestore.rules</code>, <code>js/views/bracket.js</code>, <code>js/store.js</code>, <code>sw.js</code>, <code>index.html</code>.</p>' +
        '</div>' +
        '<div style="margin-bottom:1rem;">' +
