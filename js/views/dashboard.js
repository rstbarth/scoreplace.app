    ...
    return `
        <div class="card mb-3" style="position: relative; overflow: hidden; ${venuePhotoBg ? venuePhotoBg : 'background: ' + bgGradient + ';'} color: ${_cardTextColor}; border: 1px solid ${_isLight ? 'rgba(0,0,0,0.08)' : 'transparent'}; border-radius: 12px; box-shadow: 0 4px 12px rgba(0,0,0,${_isLight ? '0.06' : '0.1'}); cursor: pointer; transition: transform 0.2s;" onclick="window.location.hash='#tournaments/${t.id}'" onmouseover="this.style.transform='translateX(5px)'" onmouseout="this.style.transform='none'">
          ...
          <div class="card-body p-4">
            ...
            <!-- Organizer -->
            <span style="display:inline-flex;align-items:center;gap:5px;font-size:0.75rem;color:rgba(255,255,255,0.65);margin-top:4px;"><img src="${(window._playerPhotoCache && window._playerPhotoCache[(t.organizerName || '').toLowerCase()] && window._playerPhotoCache[(t.organizerName || '').toLowerCase()].indexOf('dicebear.com') === -1) ? window._playerPhotoCache[(t.organizerName || '').toLowerCase()] : 'https://api.dicebear.com/9.x/initials/svg?seed=' + encodeURIComponent(t.organizerName || t.organizerEmail || '') + '&backgroundColor=c0aede,d1d4f9,b6e3f4,ffd5dc,ffdfbf'}" onerror="this.onerror=null;this.src='https://api.dicebear.com/9.x/initials/svg?seed=${encodeURIComponent(t.organizerName || t.organizerEmail || '')}&backgroundColor=c0aede,d1d4f9,b6e3f4,ffd5dc,ffdfbf'" data-player-name="${window._safeHtml(t.organizerName || '')}" style="width:20px;height:20px;border-radius:50%;object-fit:cover;flex-shrink:0;" />${window._safeHtml(t.organizerName || t.organizerEmail || '')} <svg width="14" height="14" viewBox="0 0 24 24" fill="rgba(251,191,36,0.8)" style="flex-shrink:0;"><path d="M2 20h20v2H2zM4 17l2-9 4 4 2-6 2 6 4-4 2 9z"/></svg></span>
