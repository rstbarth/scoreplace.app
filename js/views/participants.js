    ...
    cardsStr = parts.map((p, idx) => {
      ...
      if (isTeam) {
        pNameHtml = pName.split('/').map((n, i) => {
          ...
          return `<div style="display:flex;align-items:center;gap:6px;margin-bottom:2px;overflow:hidden;"><img src="${_mPhoto}" ${_mErr} data-player-name="${_nmH}" style="width:${i === 0 ? '24px' : '20px'};height:${i === 0 ? '24px' : '20px'};border-radius:50%;object-fit:cover;flex-shrink:0;"><span ${_editAttr}>${_nmH}</span></div>`;
        }).join('');
      } else {
